...
cmd = './gbdt -t 10 -s {nr_thread} te.gbdt.dense te.gbdt.sparse tr.gbdt.dense tr.gbdt.sparse te.gbdt.out tr.gbdt.out'.format(nr_thread=NR_THREAD) 
print(cmd)
subprocess.call(cmd, shell=True)
...
cmd = './ffm-train -k 4 -t 5 -l 0.000005 -s {nr_thread} tr.ffm model'.format(nr_thread=NR_THREAD) 
print(cmd)
subprocess.call(cmd, shell=True)
